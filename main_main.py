...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)  # adjust num_workers if needed
    ...
    #diffusion = torch.nn.DataParallel(diffusion, device_ids=[0, 1])
    diffusion.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            optimizer.zero_grad()
            videos = batch['pixel_values'].to(device).float()
            #print(videos.shape)
            t = torch.randint(0, diffusion.num_timesteps, (videos.size(0),), device=videos.device)
            #text_embeds = encode_text("chatgpot is great", batch_size)
            if use_amp:
                with torch.cuda.amp.autocast():
                    loss = diffusion(videos, t)#, cond=text_embeds)
